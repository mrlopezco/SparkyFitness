...
No `DB_PATH` / `SERVER_BACKUP_PATH` / `SERVER_UPLOADS_PATH` / `GHD_DATA_PATH` env vars. The compose file declares Docker named volumes on the Coolify host:
...
| `sparkyfitness-ghd-data` | GHD sidecar `/data` (tokens + SQLite per user) |
...
- **Do not put `NODE_ENV` in the Coolify env UI or compose.** Runtime `NODE_ENV=production` is set in `docker/Dockerfile.backend`. Builder stages force a full pnpm install even if Coolify still passes a leftover build ARG. After removing `NODE_ENV` from compose, refresh the compose in Coolify, then delete the UI entry.
...
| `sparkyfitness-ghd` | Build `docker/Dockerfile.ghd_microservice` (repo-root context) → tag `sparkyfitness-ghd:local` |
...
- GHD sidecar data lives on the `sparkyfitness-ghd-data` named volume; if the Node API cannot reach it, check `GHD_MICROSERVICE_URL` (default `http://sparkyfitness-ghd:8001`) and `sparkyfitness-ghd` logs. This fork does not run `sparkyfitness-garmin`.
