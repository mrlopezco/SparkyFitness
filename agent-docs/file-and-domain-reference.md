...
| **Training Plan** (fork) | `routes/trainingPlanRoutes.ts`, `services/trainingPlanService.ts` `trainingPlanAiService.ts` `trainingAthleteSnapshotService.ts` `trainingAdherenceService.ts`, `models/trainingPlanRepository.ts` | `pages/Training/` `api/Training/` `hooks/Training/` | — | `schemas/api/TrainingPlan.api.zod.ts` + `schemas/database/Training*.zod.ts` |
