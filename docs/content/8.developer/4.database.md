...
| `training_plans` | Date-based training plans (fork Training module) |
| `training_goals` | Race / weight / habit goals for a training plan |
| `training_commitments` | Fixed constraints (soccer, hikes) the plan must respect |
| `training_plan_sessions` | Date-keyed planned sessions |
| `training_session_completions` | Links planned sessions to logged exercise entries; athlete_execution_score + ai_review |
| `training_athlete_snapshots` | Compact athlete context for AI (code-built) |
| `training_coach_sessions` | Training coach conversation threads |
| `training_coach_messages` | Coach session messages |
| `training_coach_session_summaries` | Rolling coach session summaries |
| `training_coach_memories` | Durable coach memories / constraints |
| `nutrition_coach_sessions` | Nutrition habit coach check-in threads |
| `nutrition_coach_messages` | Nutrition coach session messages |
| `nutrition_coach_session_summaries` | Rolling nutrition coach session summaries |
| `nutrition_coach_memories` | Durable nutrition coach memories / commitments |
| `nutrition_coach_context_snapshots` | Code-built diary + activity context for the nutrition coach |
| `training_fitness_tests` | Periodic fitness tests / snapshots (time trials, Cooper, etc.) |
...
| `ghd_sync_runs` | Fork: Garmin Health Data sync run audit (keep-alive / manual) |
| `ghd_activity_map` | Fork: Garmin activity id → `exercise_entries` projection map |
| `ghd_history_import_jobs` | Fork: GHD history gap-fill jobs (7-day chunked) |
| `ghd_history_import_weeks` | Fork: per-week coverage for a GHD history import job |
...
| `user_module_preferences` | Fork: per-user module visibility toggles (nav/route gating) |
