...
#### D. Training plan & nutrition coach (fork; `create_diary_policy` — writable/readable by delegates with `can_manage_diary`, readable with `can_view_reports`)

| Table Name | Description | Write (insert/update/delete) | Read (select) |
| :--- | :--- | :--- | :--- |
| `nutrition_coach_sessions` | Nutrition habit coach check-in threads | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `nutrition_coach_messages` | Nutrition coach session messages (inherits session access) | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `nutrition_coach_session_summaries` | Rolling nutrition coach session summaries | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `nutrition_coach_memories` | Durable nutrition coach memories / commitments | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `nutrition_coach_context_snapshots` | Code-built diary + activity context for the nutrition coach | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
