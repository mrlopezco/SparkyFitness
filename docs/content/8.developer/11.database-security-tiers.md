...
| `user_module_preferences` | Fork: per-user module visibility toggles (nav/route gating for exercises, medications, etc.) | Owner-Only | Owner-Only |
| `ghd_sync_runs` | Fork: Garmin Health Data sync run audit (keep-alive / manual ranges) | Owner-Only | Owner-Only |
| `ghd_activity_map` | Fork: Garmin activity id → exercise entry projection map | Owner-Only | Owner-Only |
| `ghd_history_import_jobs` | Fork: GHD history gap-fill job state (chunked backfill) | Owner-Only | Owner-Only |
| `ghd_history_import_weeks` | Fork: per-week coverage rows for a GHD history import job | Owner-Only | Owner-Only |
...
| `training_plans` | Date-based training plans (goals, horizon, status) | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_goals` | Race / body-weight / habit goals for a training plan | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_commitments` | Fixed calendar constraints (soccer, hikes, travel) | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_plan_sessions` | Date-keyed planned training sessions | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_session_completions` | Planned-vs-actual adherence links to exercise entries | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_athlete_snapshots` | Code-built athlete context summaries for AI coach | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_coach_sessions` | Training coach conversation threads | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_coach_messages` | Messages within a training coach session | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_coach_session_summaries` | Rolling summaries of coach sessions | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_coach_memories` | Durable coach facts (constraints, preferences, injuries) | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
| `training_fitness_tests` | Periodic fitness snapshots / time trials the athlete reports | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
