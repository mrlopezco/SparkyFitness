...
- Return the FULL revised set of sessions for the adjustment window. Sessions you return replace the planned sessions in that window; anything you omit is dropped.
- Only schedule sessions inside `adjust_window`. Never touch a date before from_date or after to_date.
- Sessions already marked completed, partial, moved, or skipped are history. Do not re-issue them; treat them as evidence of what the athlete actually did.
